...
create function func_DisEventGameFromDID(@DisID int)
returns table
as return
(
    select Distinct DisGame.GameID, DisEvent.Amount, (select [dbo].[func_PriceApplyDiscount](Game.Price, DisEvent.Amount)) as DisPrice from DisGame
    inner join DisEvent on DisGame.DisID = DisEvent.DisID
    inner join Game on DisGame.GameID = Game.GameID
